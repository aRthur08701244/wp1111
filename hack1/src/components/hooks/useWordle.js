...
const useWordle = (solution) => {
    ...
    // const [solSet, setSolSet] = useState({0:solution[0], 1:solution[1], 2:solution[2], 3:solution[3], 4:solution[4], 5:solution[5]})


    // setGuesses([[{char:'c', color:'grey'},{char:'o', color:'grey'},{char:'d', color:'grey'},{char:'e', color:'yellow'},{char:'s', color:'grey'}],[{char:'c', color:'grey'},{char:'o', color:'grey'},{char:'d', color:'grey'},{char:'e', color:'yellow'},{char:'s', color:'grey'}],[{char:'c', color:'grey'},{char:'o', color:'grey'},{char:'d', color:'grey'},{char:'e', color:'yellow'},{char:'s', color:'grey'}],[{char:'c', color:'grey'},{char:'o', color:'grey'},{char:'d', color:'grey'},{char:'e', color:'yellow'},{char:'s', color:'grey'}],[{char:'c', color:'grey'},{char:'o', color:'grey'},{char:'d', color:'grey'},{char:'e', color:'yellow'},{char:'s', color:'grey'}],[{char:'c', color:'grey'},{char:'o', color:'grey'},{char:'d', color:'grey'},{char:'e', color:'yellow'},{char:'s', color:'grey'}]])

    // setCurGuess([{char:'c', color:'grey'},{char:'o', color:'grey'},{char:'d', color:'grey'},{char:'e', color:'yellow'},{char:'s', color:'grey'}])
    ...
    const handleEnter = () => {
        ...
        let solSet = solution.split('')
        
        const addUsedChars = (newUsedChar, color, originalUsedChars) => {
            if (!originalUsedChars[newUsedChar]) originalUsedChars[newUsedChar] = color
            else {
                if ((originalUsedChars[newUsedChar] == 'yellow') && (color == 'green')) {
                    originalUsedChars[newUsedChar] = color
                }
            }
            return originalUsedChars
        }

        const replaceGuess = (turn, guess, originalguesses) => {
            originalguesses[turn] = guess
            return originalguesses
        }
        
        let createGuess = Array(5)
        for (let i = 0; i < curGuess.length; i++) { 
            if (solution[i] == curGuess[i]) {
                createGuess[i] = {char: curGuess[i], color:'green'}
                solSet.splice(solSet.indexOf(curGuess[i]), 1)
                setUsedChars(addUsedChars(curGuess[i], 'green', usedChars))
            }
        }

        for (let i = 0; i < curGuess.length; i++) { 
            if (createGuess[i]) continue
            if (solSet.indexOf(curGuess[i]) > -1) {
                createGuess[i] = {char: curGuess[i], color:'yellow'}
                solSet.splice(solSet.indexOf(curGuess[i]), 1)
                setUsedChars(addUsedChars(curGuess[i], 'yellow', usedChars))
            }
            else {
                createGuess[i] = {char: curGuess[i], color:'grey'}
            }
        }

        setGuesses((guesses) => replaceGuess(turn, createGuess, guesses))
        setCurGuess('')
        setTurn(turn+1)
        ...
        if (solution === curGuess) setIsCorrect(true)
        
        
        // 5-2) usedChars update
        // console.log(usedChars)
        // setUsedChars({'a': 'yellow'})
